...
evalPrim [] = eval Void
evalPrim [x] = eval x
--evalPrim [Pair x xs] = eval (Pair x xs)
...
isList [x] = case x of 
  ...
  Pair _ xs -> do b <- isList [xs]
                  if lowerBool b then return (Boolean True) else return (Boolean False)
...
isNumber [Number x] = return (Boolean True)
isNumber [x] = return (Boolean False)
...
isBoolean [x] = return (Boolean False)
